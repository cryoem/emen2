	...
	$.extend({
		jsonRPC: {
			...
			request: function(method, params, callbacks, url) {
				...
				return this._doRequest(JSON.stringify(this._requestDataObj(method, params, 1)),
					callbacks,
					url);
			...
			call: function(method, params, callback, errback) {
				...
				return this.request(method, params, {success:function(u){callback(u.result)}, error: function(u){errback(u.error)}});
			},
			...
			batchRequest: function(requests, callbacks, url) {
				...
				if(!$.isArray(requests) || requests.length === 0) {
					throw("Invalid requests supplied for jsonRPC batchRequest. Must be an array object that contains at least a method attribute");
				}

				// Make sure each of our request objects are valid
				var self = this;
				$.each(requests, function(i, req) {
					self._validateRequestMethod(req.method);
					self._validateRequestParams(req.params);
				});
				this._validateRequestCallbacks(callbacks);
			...
			_validateRequestCallbacks: function(callbacks) {
				...
				if(typeof(callbacks) !== 'undefined') {
					if(typeof(callbacks.error) !== 'undefined' && typeof(callbacks.error) !== 'function') {
						throw("Invalid error callback supplied for jsonRPC request");
					}
					if(typeof(callbacks.success) !== 'undefined' && typeof(callbacks.success) !== 'function') {
			...
			_doRequest: function(data, callbacks, url) {
				...
				return $.ajax({
					...
					error: function(jqXHR, textStatus, errorThrown) {
						self._requestError(callbacks, jqXHR, textStatus, errorThrown);
					},
					success: function(json, textStatus, jqXHR) {
						self._requestSuccess(callbacks, json);
					}
				});
			},
			...
			_requestError: function(callbacks, jqXHR, textStatus, errorThrown) {
				if (textStatus == 'abort') {
					// ignore aborted requests.
					// console.log("Aborted JSONRPC request");
					return
				}
				if (typeof(callbacks) !== 'undefined' && typeof(callbacks.error) === 'function') {
					callbacks.error(this._response(jqXHR.responseText));
				}
			},
