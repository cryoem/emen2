...
class Record(dict) :
    ...
    def __attr_setter(attr, typ=str, default=''): #@NoSelf
        def prop(self, value):
            try:
                value= typ(value)
            except TypeError:
                if hasattr(value, '__call__'):
                    ...
                    value = default
            self.__metadata[attr] =value 
        return prop
    
    metadata = property(lambda self: DictProxy(self.__metadata))
    recid = property(lambda self: self.metadata.get('recid'), __attr_setter('recid', int,0))
    rectype = property(lambda self: self.metadata.get('rectype', ""), __attr_setter('rectype'))
    __creator = property(lambda self: self.metadata.get('creator', 0), __attr_setter('creator'))
    __creationtime = property(lambda self: self.metadata.get('creationtime'), __attr_setter('creationtime'))
    __permissions = property(lambda self: self.metadata.get('permissions', ((),(),(),())), 
                                          __attr_setter('permissions', tuple, lambda self: self.__metadata.get('permissions') ))
    __params = property(lambda self: self)
    ...
    def __getstate__(self):
        ...
        odict['metadata'] = self.__metadata
        del odict['_Record__metadata']
        odict['data'] = dict(self)
    ...
    def __setstate__(self,dct):
        """restore unpickled values to defaults after unpickling"""
        self.__metadata = {}
        if dct.has_key('metadata'):
            self.__metadata.update(dct.pop('metadata'))
        for idx, save in ( (key, key.split('_')[-1])
                                 for key in dct.copy()
                                 if key.split('_')[-1] in self.param_special):
            self.__metadata[save] = dct.pop(idx)
        
        if dct.has_key('data'):
            self.update(dct.pop('data'))
        if dct.has_key('_Record__params'):
            self.update(dct.pop('_Record__params'))
        
        if dct.has_key("localcpy") :
            del dct["localcpy"]
    ...
    def addcomment(self, value):
        if not isinstance(value,basestring):
            ...
            self.__metadata['comments'].append(value)  # store the comment string itself       
    
            for i,j in d.items():
                if i == 'comments':
                    print "Warning: cannot set comments inside a comment"
                self.__setitem__(i,j)
    __comments = property(lambda self: self.__metadata.get('comments'), addcomment)
    ...
    def __setitem__(self,key,value):
        ...
        if value == None or str(value).lower == "none" : 
...
#        return key in self.keys()
    ...
    def changedparams(self):
        print self.__oparams
        cp = []
        for k,v in self.items():
            if v == self.__oparams.get(k):
                continue
            else:
                cp.append(k)
        return cp       
    ...
    def validate(self):
        ...
        if self.keys():
            pds=self.__context.db.getparamdefs(self.keys())
