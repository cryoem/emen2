...
class XMLRPCResourceOld(xmlrpc.XMLRPC):
	...
	def _getFunction(self, request, db):
		request.content.seek(0, 0)
		content = request.content.read()
		args, functionPath = xmlrpclib.loads(content)
		host = request.getClientIP()
		print """---- [%s] [%s] ---- xmlrpc request: %s ----"""  % \
				(time.strftime("%Y/%m/%d %H:%M:%S"),host,functionPath)
		path = functionPath.split('.')
		func = database.Database.publicmethods[path[0]]
		if len(path) > 1:
			for x in path[1:]:
				func = getattr(func, x)
		request.setHeader("content-type", "text/xml")
		print 'PATH:::',path
		if path[-1] != u'__repr__':
			print 'aqaaaaaaaaaaaaaa'
			result = func(db, host=host, *args)
		else:
			result = func()
	...
	def render(self, request):
		d = threads.deferToThread(self._getFunction, request)
