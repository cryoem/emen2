...
def get_path_id(path, cur_dir=None, dbinfo=None):
    ...
    try:
        ...
        return emen2.Database.SecurityError('login required')
...
def get_child_id(name, db, ctxid, cur_dir, host=None):
    ...
    children = db.getchildren(cur_dir, keytype='record', ctxid=ctxid)
...
def get_root_id(db, ctxid, host=None):
    ...
    return min(db.getindexbyrecorddef('folder', ctxid=ctxid))
